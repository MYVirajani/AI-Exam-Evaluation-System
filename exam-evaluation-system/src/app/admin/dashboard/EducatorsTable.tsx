...
export default function EducatorsTable() {
  ...
  const handleExport = async () => {
    try {
      ...
      link.setAttribute("download", `educators_export_${new Date().toISOString().split('T')[0]}.csv`);
        ...
        {table.getRowModel().rows.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <p className="text-gray-500 mb-4">
              {globalFilter ? "Try adjusting your search criteria" : "Get started by adding your first educator"}
            </p>
            {!globalFilter && (
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4 px-2 py-4 bg-gray-50 border-t border-gray-200">
          ...
          <div className="flex items-center gap-2 text-sm text-gray-700 font-medium">
            <span>
              ...
              {globalFilter && ` (filtered from ${educators.length} total entries)`}
            </span>
          </div>
