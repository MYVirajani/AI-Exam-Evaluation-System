...
# def embed_student_answers(provider: str, model: str, module_code: str, year: int, month: str):
#     embedder = OpenAIEmbedder(model) if provider == "OpenAI" else GeminiEmbedder(model)
#     db_service = StudentAnswerService(provider_suffix=provider)
...
def embed_student_answers(provider: str, model: str, module_code: str, year: int, month: str):
    ...
    # Step 2: Normalize provider suffix for DB routing
    provider_suffix = embedder.get_table_suffix()  # "openai" or "gemini"

    # Step 3: Use correct relational DB table for fetching answers
    db_service = StudentAnswerService(provider_suffix=provider_suffix)

    # Step 4: Fetch all answers for the specified module, year, and month
    grouped_answers = db_service.get_all_answers_grouped(module_code=module_code, year=year, month=month)

    # Step 5: Save embeddings into correct vector DB table
    vector_db = StudentAnswerEmbeddingDB(embedder)
    for key, answers in grouped_answers.items():
        vector_db.save_embeddings(answers)
