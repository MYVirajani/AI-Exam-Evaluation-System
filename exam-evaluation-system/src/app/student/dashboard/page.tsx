...
const StudentHomePage: React.FC = () => {
  ...
  // ✅ FIXED: Navigate to assessment detail page with type-based routing
  const handleEventCardClick = (
    ...
    if (assessmentType === 'bubbleSheet') {
      // Bubble sheet - go to bubble sheet page (NO handwritten checkbox)
      route = `/student/assessments/${assessmentId}/bubblesheet?studentId=${userId}&moduleId=${moduleId}`;
      console.log('🎯 Routing to Bubble Sheet page:', route);
    } else if (assessmentType === 'quiz') {
      // Quiz - go to quiz page
      route = `/student/quiz/${assessmentId}?studentId=${userId}&moduleId=${moduleId}`;
      console.log('📝 Routing to Quiz page:', route);
    } else {
      // Regular assessment - go to regular page (HAS handwritten checkbox)
      route = `/student/assessments/${assessmentId}?studentId=${userId}&moduleId=${moduleId}`;
      console.log('📝 Routing to Regular Assessment page:', route);
    }
              ...
              >
                {allAssessments.map((assess) => (
                  <StudentEventCard
                    ...
                    duration={Number(assess.duration)}
                    open_at={assess.open_at}
                    close_at={assess.close_at}
                    deadline={assess.deadline}
                    assessmentType={assess.type} // ✅ ADDED: Pass assessment type
                    onClick={() =>
