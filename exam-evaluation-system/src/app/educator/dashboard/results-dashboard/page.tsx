...
const StudentResultsDashboard = () => {
  ...
  const [filteredGradedAnswers, setFilteredGradedAnswers] = useState<GradedAnswer[]>([]);
  ...
  const assessmentTitle = searchParams.get('title');
  ...
  const fetchData = async (model: ModelType) => {
    try {
      ...
      const allGradedAnswers = Array.isArray(gradedAnswersData) ? gradedAnswersData : [];

      setResults(allResults);
      setGradedAnswers(allGradedAnswers);

      // Filter data if assessmentId is provided
      if (assessmentId) {
        const filtered = allResults.filter((result: Result) => 
          result.assessment_id === assessmentId
        );
        const filteredGraded = allGradedAnswers.filter((answer: GradedAnswer) => 
          answer.assessment_id === assessmentId
        );
        
        setFilteredResults(filtered);
        setFilteredGradedAnswers(filteredGraded);
      } else {
        setFilteredResults(allResults);
        setFilteredGradedAnswers(allGradedAnswers);
      }
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between mb-8">
            <div>
              <h1 className="text-4xl font-bold mb-4 lg:mb-0 text-center lg:text-left bg-gradient-to-r from-blue-600 to-purple-600 bg-clip-text text-transparent">
                {assessmentId ? 'Assessment Results' : 'Student Results Dashboard'}
              </h1>
              
              {/* Assessment Info */}
              {assessmentId && (assessmentTitle || moduleTitle) && (
                <div className="mt-2 p-3 bg-blue-50 rounded-lg border border-blue-200">
                  <div className="flex items-start space-x-2">
                    <FiInfo className="w-4 h-4 text-blue-600 mt-0.5 flex-shrink-0" />
                    <div className="text-sm">
                      <div className="font-medium text-blue-900">
                        {assessmentTitle && `Assessment: ${decodeURIComponent(assessmentTitle)}`}
                      </div>
                      {moduleTitle && (
                        <div className="text-blue-700 mt-1">
                          Module: {decodeURIComponent(moduleTitle)}
                        </div>
                      )}
                    </div>
                  </div>
            ...
            <div className="flex items-center justify-center space-x-2 text-sm text-gray-600 bg-gray-50 rounded-lg py-2 px-4">
              <div className="flex items-center space-x-2">
                ...
                {assessmentId && (
                  <>
                    <span>•</span>
                    <span>Filtered by assessment</span>
                  </>
                )}
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8 p-4 bg-gray-50 rounded-lg">
            <div className="text-center">
              ...
              <div className="text-sm text-gray-600">
                {assessmentId ? 'Assessment Results' : 'Total Results'} ({getModelDisplayName(selectedModel)})
              </div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold text-purple-600">{filteredGradedAnswers.length}</div>
              <div className="text-sm text-gray-600">
                {assessmentId ? 'Graded Submissions' : 'Graded Answers'} ({getModelDisplayName(selectedModel)})
              </div>
            </div>
          </div>

          {/* No Results Message */}
          {assessmentId && filteredResults.length === 0 && filteredGradedAnswers.length === 0 && !loading && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 max-w-sm mx-auto">
                No results found for this assessment. Students may not have submitted their work yet, or the results haven't been processed.
              </p>
            </div>
          )}

          {/* Tab Navigation */}
          {(filteredResults.length > 0 || filteredGradedAnswers.length > 0) && (
            <>
                  ...
                  <>
                    {filteredResults.length > 0 ? (
                      ...
                      <div className="text-center py-8">
                        <p className="text-gray-600">No summary results available for this assessment.</p>
                      </div>
                    )}
                  </>
                )}
                {activeTab === 'graded' && (
                  <>
                    {filteredGradedAnswers.length > 0 ? (
                      <GradedAnswersTable gradedAnswers={filteredGradedAnswers} />
                    ) : (
                      <div className="text-center py-8">
                        <p className="text-gray-600">No detailed graded answers available for this assessment.</p>
                      </div>
                    )}
                  </>
                )}
