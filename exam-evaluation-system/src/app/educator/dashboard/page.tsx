...
const FALLBACK_IMAGES = Array.from({ length: 13 }, (_, i) => (
  `/background-images/image${i+1}.jpg`
))
...
export default function EducatorHomePage() {
  ...
  const handleCreateEvent = async (data: EventFormData) => {
    ...
    if (data.questionPaper?.length) form.append("questionPaper", data.questionPaper[0])

    const res = await fetch("/api/educator/assessment", { method: "POST", body: form })
    const json = await res.json()
  ...
  useEffect(() => {
    async function loadData() {
      try {
        ...
        const { modules, assessments }: { modules: ModuleAPI[]; assessments: AssessmentAPI[] } =
          await res.json()
  ...
  if (loading) {
    return (
      <div className="flex items-center justify-center h-screen">
        ...
        <span className="ml-2 text-lg text-blue-900">Loading modules & events…</span>
      </div>
    )
  }
