...
// export async function POST(request: NextRequest) {
//   try {
//     const { selectedModel, moduleId, assessmentId, evaluationType, parameters, selectedSubmissions } = await request.json()
...
//     //   assessment_id: assessmentId, // pass it to Flask
//     //   selected_submissions: selectedSubmissions || null, // pass selection
...
//       module_code: moduleId,
//       assessment_id: assessmentId,
//       selected_submissions: selectedSubmissions || null,
//       year,
//       month,
//       ...parameters
//     };
...
export async function POST(request: NextRequest) {
  try {
    const { selectedModel, moduleId, assessmentId, selectedSubmissions, parameters } = await request.json()

    console.log(`Starting evaluation with model: ${selectedModel}`)
    console.log(`Module ID: ${moduleId}, Assessment ID: ${assessmentId}`)
    console.log(`Selected submissions: ${selectedSubmissions?.length} submissions`)
    ...
    const flaskEndpoint = `${FLASK_API_BASE}/api/run-full-evaluation`
    const taskDescription = 'Running complete evaluation pipeline'
