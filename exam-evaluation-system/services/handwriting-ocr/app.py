...
# # # #     def convert_handwritten_pdf(self, pdf_bytes: bytes) -> Tuple[str, str, float]:
# # # #         """Main conversion function that returns extracted text and creates text-only PDF"""
# # # #         if not self.trocr_loaded:
# # # #             return "", "TrOCR not available", 0.0
        
# # # #         try:
# # # #             # Convert PDF to images
# # # #             page_images = self.pdf_to_images(pdf_bytes, dpi=350)
# # # #             if not page_images:
# # # #                 return "", "Failed to convert PDF to images", 0.0
...
# # # #                 print(f"Page {page_num + 1}: Detected {len(text_regions)} text regions")
...
# # # #                 print(f"Page {page_num + 1}: Extracted {len(page_text_lines)} valid lines")
...
# # # #         except Exception as e:
# # # #             print(f"Error in convert_handwritten_pdf: {e}")
# # # #             import traceback
# # # #             traceback.print_exc()
...
# # # #         extracted_text, error_msg, confidence = converter.convert_handwritten_pdf(pdf_bytes)
        
# # # #         if error_msg:
# # # #             return jsonify({'success': False, 'error': error_msg})
        
# # # #         if not extracted_text.strip():
# # # #             return jsonify({'success': False, 'error': 'No text could be extracted from the handwritten PDF'})
...
# # # #                 temp_pdf_path,
# # # #                 as_attachment=True,
# # # #                 download_name='converted_text.pdf',
...
# # # #     except Exception as e:
# # # #         print(f"Error in convert_handwritten: {e}")
# # # #         import traceback
# # # #         traceback.print_exc()
...
# # #             else:
# # #                 return "", f"Unsupported file format: {filename}", 0.0
...
# # #         if not (converter.is_pdf_file(filename) or converter.is_image_file(filename)):
# # #             return jsonify({'success': False, 'error': 'File must be a PDF, PNG, JPG, or JPEG'})
...
# #         print(f"DEBUG: is_image_file('{filename}') = {result}")
# #         return result
...
# #         print(f"DEBUG: is_pdf_file('{filename}') = {result}")
# #         return result
...
# #         is_pdf = converter.is_pdf_file(filename_lower)
# #         is_image = converter.is_image_file(filename_lower)
        
# #         print(f"PDF check: {is_pdf}")
# #         print(f"Image check: {is_image}")
...
# #         # Create text-only PDF
# #         if converter.create_text_pdf(extracted_text, temp_pdf_path):
# #             print(f"✓ PDF created successfully: {temp_pdf_path}")
# #             return send_file(
# #                 temp_pdf_path,
# #                 as_attachment=True,
# #                 download_name='converted_text.pdf',
...
#         # CRITICAL FIX: Accept both PDFs and images
#         if not (is_pdf or is_image):
#             error_msg = f'File must be a PDF, PNG, JPG, or JPEG. Received: {filename} (content-type: {file.content_type})'
#             print(f"✗ File type validation failed: {error_msg}")
#             return jsonify({'success': False, 'error': error_msg})
