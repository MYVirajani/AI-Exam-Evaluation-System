...
class LectureMaterialEmbeddingDB(BaseVectorDBService):
    ...
    def save_chunks(self, chunks: list[LectureChunk], assessment_id: str = None) -> None:
        ...
        self.commit()
        logger.info("Saved %d lecture chunks for assessment %s.", len(chunks), assessment_id)
    ...
    def document_exists(self, module_code: str, source_file: str, assessment_id: str = None) -> bool:
        """
        Checks whether any chunks exist for the given document and assessment in the vector table.
        Avoids re-embedding if already present.
        """
        if assessment_id:
            self.cursor.execute(
                f"""
                SELECT 1 FROM {self.table}
                WHERE module_code = %s AND source_file = %s AND assessment_id = %s
                LIMIT 1;
                """,
                (module_code, source_file, assessment_id),
            )
        else:
            self.cursor.execute(
                f"""
                SELECT 1 FROM {self.table}
                WHERE module_code = %s AND source_file = %s AND assessment_id IS NULL
                LIMIT 1;
                """,
                (module_code, source_file),
            )
