...
import { FiTrash2, FiFileText, FiFile } from "react-icons/fi";
...
const QuizForm: React.FC<QuizFormProps> = ({ questions, setQuestions }) => {
...
export default function AssessmentFormPage() {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      doc.text(title || 'Quiz', pageWidth / 2, yPosition, { align: 'center' });
      yPosition += 15;
      ...
      const fileName = `${title.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'quiz'}_question_paper.pdf`;
      doc.save(fileName);
    } catch (error) {
  ...
  const generateDOCX = async () => {
    ...
    try {
      ...
      children.push(
        new Paragraph({
          children: [new TextRun({ text: title || 'Quiz', bold: true, size: 32 })],
      ...
      const fileName = `${title.replace(/[^a-z0-9]/gi, '_').toLowerCase() || 'quiz'}_question_paper.docx`;
  ...
  const handleSubmit = async () => {
    ...
    // await fetch("/api/educator/quiz", { method: "POST", body: JSON.stringify(quiz) });
      ...
      <div className="max-w-4xl mx-auto px-6 py-8">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6 mb-6">
          ...
          <p className="text-gray-600">
            Design your quiz with MCQ and short answer questions, then download as PDF or DOCX format
          </p>
        </div>
