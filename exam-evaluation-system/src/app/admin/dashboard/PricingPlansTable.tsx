...
export default function PricingPlansTable() {
  ...
  const handleExport = async () => {
    try {
      ...
      link.setAttribute("download", `pricing_plans_export_${new Date().toISOString().split('T')[0]}.csv`);
      link.style.visibility = "hidden";
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      
      toast.success(`Exported ${currentData.length} pricing plans successfully`);
    } catch (error) {
      ...
      cell: ({ row }) => (
        <div className="text-gray-900">
          <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-purple-100 text-purple-800">
            {BILLING_PERIOD_LABELS[row.original.billing_period] ?? row.original.billing_period}
          </span>
        </div>
      ),
    },
        ...
        <div className="text-sm">
          <ul className="list-disc list-inside text-gray-700 space-y-1">
            {row.original.features.map((f, i) => (
              <li key={i} className="break-words" title={f}>{f}</li>
            ))}
          </ul>
        </div>
      ) : (
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            title={row.original.subscriptionCount > 0 ? "Cannot delete plan with active subscriptions" : "Delete plan"}
          >
            Delete
          </button>
        </div>
      ...
      <div className="bg-white border border-gray-200 rounded-lg overflow-hidden shadow-sm">
        ...
        {table.getRowModel().rows.length === 0 && !isLoading && (
          <div className="text-center py-12">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">No pricing plans found</h3>
            <p className="text-gray-500 mb-4">
              {globalFilter ? "Try adjusting your search criteria" : "Get started by adding your first pricing plan"}
            </p>
            {!globalFilter && (
              <button 
                onClick={() => {
